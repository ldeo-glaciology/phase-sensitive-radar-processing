function site = fmcw_dmelt(filename2)

% site = fmcw_dmelt(filename1,filename2,cfg)
% FMCW Radar Processor for vertical strain rate and melt
% from a pair of bursts. Modified from fmcw_melt to use new differential
% processing 22 Aug 2014.
%
% args:
...
cfg = fmcw_process_config_juneau(filename2);%fmcw_process_config_default; % Load default configuration
if nargin >2
    % overwrite defaults
...
% Load
f = fmcw_load(filename1,1); % using burst 1 only
g = fmcw_load(filename2,1);
...
cmd = [mfilename '(''' name1 ext1 ''',''' name2 ext2 ''')'];
clipboard('copy',cmd)
Disp(cmd)
...
if numel(attSetList)>1
    jj = 2;
    disp(['Warning: multiple attenuator settings found file ' name1 '. Keeping ' num2str(attSetList(jj)) ' first set only.'])
    fs = fmcw_burst_split_by_att(f);
    f = fs(jj); % keeing results from first attenuator setting only.
end
attSetList = unique(g.chirpAtt,'stable');
if numel(attSetList)>1
    jj = 2;
    disp(['Warning: multiple attenuator settings found file ' name2 '. Keeping ' num2str(attSetList(jj)) ' set only.'])
...
F = fmcw_drange(fo,cfg.p,cfg.maxRange,cfg.winFun);
fm = fmcw_burst_mean(fo);
f = fmcw_drange(fm,cfg.p,cfg.maxRange,cfg.winFun); % note - should this be changed to a weighted mean for cases where the attenuation changes within the burst
%[g.rangeCoarse,g.rangeFine,g.specCor,g.specRaw] = fmcw_range(gm,cfg.p,cfg.maxRange,cfg.winFun);
G = fmcw_drange(go,cfg.p,cfg.maxRange,cfg.winFun);
gm = fmcw_burst_mean(go);
g = fmcw_drange(gm,cfg.p,cfg.maxRange,cfg.winFun);
...
switch cfg.errorMethod
    case 'emperical'
        ...
        f.phaseStdDev = std(F.specCor)./abs(f.specCor); % phase standard deviation of the burst
        f.phaseStdError = f.phaseStdDev/sqrt(size(fo.vif,1)); % phase standard error of the mean shot - using sqrt(n)
        g.phaseStdDev = std(G.specCor)./abs(g.specCor); % phase standard deviation of the burst
        g.phaseStdError = g.phaseStdDev/sqrt(size(go.vif,1)); % phase standard error of the mean shot - using sqrt(n)
...
f.rangeError = fmcw_phase2range(f.phaseStdError,f.lambdac);
g.rangeError = fmcw_phase2range(g.phaseStdError,g.lambdac);
...
if cfg.doPlotAlignFine || cfg.doPlotAll
    ...
    for ii = 1:length(range), height(ii) = abs(AF.COR(ii,AF.mci(ii)))+0.05; end
    plot3(range,AF.lagvec(AF.mci),height,'k.','markersize',30); % best amp
    %h(2) = plot3(range,AF.lagvec(AF.mci),ones(size(range)),'w.','markersize',20); % best amp
    if cfg.doSmartUnwrap
...
site.t1 = fo.TimeStamp; % time
site.t2 = go.TimeStamp; % time
site.dt = dt;
...
site.rangef = f.rangeCoarse;
site.rangeg = g.rangeCoarse;
...
ylim([min(20*log10(abs(g.specCor(ii)))) -20])

function h = erbar(x,y,eh,el,linecol,pointcol)
...
xb = [x; x];
yb = [y+eh; y+el];
plot(xb,yb,'col',linecol,'linewidth',1.5) % Plot error bars
hold on
plot(x,y,'.','col',pointcol) % Plot points
