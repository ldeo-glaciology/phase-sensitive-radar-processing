...
% Brennan, Lok, Nicholls and Corr, 2014
%
...
% spec = positive frequency half of spectrum up to (nyquist minus deltaf)
...
fs = vdat.fs; %default: 4e4
T = vdat.T;
B = vdat.B; %default: 2e8
K = vdat.K;
ci = vdat.ci; %set in fmcw_load, ci=3e8/sqrt(dielectric of ice)
fc = vdat.fc;
...
%nf = length(Rcoarse); 
win = window(winfun,N); %chebwin(N);  %rectwin(N); %win = blackman(N);
...
for ii = 1:nchirps
    ...
    fftvif = fft(vifpad);
    fftvif = (sqrt(2*p)/length(vifpad)).*fftvif; % fft and scale for padding 
