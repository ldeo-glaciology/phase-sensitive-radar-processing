...
H.Nsamples = sscanf(A(loc(1)+length(['Samples: ']):searchCR(1)+loc(1)),'%d\n');
...
fid = fopen([path,filesep,'Config.ini'],'rt'); % Compatible with Linux
if fid==-1
    fid = fopen([path,filesep,'config.ini'],'rt'); % In case file renamed to lowercase
end
